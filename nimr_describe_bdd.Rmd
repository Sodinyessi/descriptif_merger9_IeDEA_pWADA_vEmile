---
title: "\\textbf{NIMR DESCRIPTION MERGER 9}"
...
$^{2}$National Institute of Medical Research (NIMR), Lagos, Nigeria.\\
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("NIMR"))#, "NIMR", "NIMR", "NIMR", "CNHU", "NIMR", "KBTH", "NIMR", "TOKOIN", "NIMR" ))   # filtre NIMR
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - NIMR - NIMR - NIMR}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "NIMR",     "NIMR",     "NIMR")) %>%#,    "NIMR",     "CNHU",  "NIMR")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - NIMR - NIMR - NIMR**"
  )
...
#\subsubsection{Profile program : NIMR - CNHU - NIMR - JOS - KBTH}

tab02 <- base0 %>%
  filter(program %in% c("NIMR",     "CNHU",  "NIMR", "JOS",     "KBTH")) %>% #,     "NIMR",   "TOKOIN",      "UCH",  "NIMR")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : JOS - KBTH - MAKURDI - NIMR - TOKOIN - UCH - NIMR**"
  )
...
#\subsubsection{Profile program : MAKURDI - NIMR - TOKOIN - UCH - NIMR}

tab002 <- base0 %>%
  filter(program %in% c("MAKURDI",  "NIMR",   "TOKOIN",      "UCH",  "NIMR")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : MAKURDI - NIMR - TOKOIN - UCH - NIMR**"
  )
...
tab03 <- base0 %>%
  ...
  #filter(program %in% c("JOS",     "KBTH",  "MAKURDI",     "NIMR",   "TOKOIN",      "UCH",  "NIMR")) %>%
  ungroup() %>%
  tbl_summary(by = "overall", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - NIMR**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including NIMR, MAKURDI, NIMR, JOS, AKURE, and NIMR — are composed exclusively of Infected patients. In contrast, programs such as NIMR, NIMR, CNHU, KBTH, TOKOIN, and NIMR have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\newpage
\begin{landscape}
\section{Database closure dates NIMR program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("NIMR",  "NIMR",    "NIMR",   "NIMR",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
tbl_char2 <- tbl_inf_charc %>%
  filter(program %in% c("NIMR", "KBTH",  "NIMR", "TOKOIN","NIMR")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
    include = c("sex_at_birth", "hiv_type", age_med_at_enroL_yrs, "year_of_inclusion_to_cohort", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "NIMR") %>%
...
\subsubsection{Graphic NIMR - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics NIMR : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
median_val0 <- round(median(tbl_nbr_mes_pat$nbmesr_rna_v, na.rm = TRUE), 0)
q1_val0 <- round(quantile(tbl_nbr_mes_pat$nbmesr_rna_v, 0.25, na.rm = TRUE), 0)
q3_val0 <- round(quantile(tbl_nbr_mes_pat$nbmesr_rna_v, 0.75, na.rm = TRUE), 0)
```
...
\begin{center}
\textbf{Median number of measures per patient (IQR) : `r median_val0` [`r q1_val0` - `r q3_val0`].}
\end{center}
...
gpl_cd4_2 <- wa_apin_full_m9_ped_apin %>%
  filter(enrol_y >= 2009 & enrol_y < 2022) %>%
  group_by(enrol_y) %>%
...
median_val1 <- round(median(tbl_nbr_mes_pat$nbmesr_rna_v, na.rm = TRUE), 0)
q1_val1 <- round(quantile(tbl_nbr_mes_pat$nbmesr_rna_v, 0.25, na.rm = TRUE), 0)
q3_val1 <- round(quantile(tbl_nbr_mes_pat$nbmesr_rna_v, 0.75, na.rm = TRUE), 0)
```

\begin{center}
\textbf{Median number of measures per patient (IQR) : `r median_val1` [`r q1_val1` - `r q3_val1`].}
\end{center}
...
graph_m6 <- bdd_graph_m6 %>%
  ...
  filter(y_art_m6 >= 2011 & y_art_m6 <= 2022) %>%
  group_by(y_art_m6) %>%
  summarise(
...
graph_m12_data <- bdd_graph_m12 %>%
  #filter(program == "ABEOKUTA") %>%
  mutate(f_art_year = year(f_art_d)) %>%
  filter(y_art_m12 >= 2011 & y_art_m12 <= 2022) %>%
  group_by(y_art_m12) %>%
  summarise(
...
wh_plot <- ggplot(df_long, aes(x = year, y = count, fill = indicator)) +
  ...
  scale_x_continuous(
    breaks = 2009:2023,       # affiche toutes les années
    limits = c(2009, 2023)    # borne l’axe X à 2006–2024
  ) +
  theme(
...
median_val2 <- round(median(tbl_nbr_vis_pat$nbvis_pat, na.rm = TRUE), 0)
q1_val2 <- round(quantile(tbl_nbr_vis_pat$nbvis_pat, 0.25, na.rm = TRUE), 0)
q3_val2 <- round(quantile(tbl_nbr_vis_pat$nbvis_pat, 0.75, na.rm = TRUE), 0)
```

\begin{center}
\textbf{Median number of visits per patient (IQR) : `r median_val2` [`r q1_val2` - `r q3_val2`].}
\end{center}
...
median_val3 <- round(median(tbl_nbr_lab_bp_pat$nb_mesur_pat, na.rm = TRUE), 0)
q1_val3 <- round(quantile(tbl_nbr_lab_bp_pat$nb_mesur_pat, 0.25, na.rm = TRUE), 0)
q3_val3 <- round(quantile(tbl_nbr_lab_bp_pat$nb_mesur_pat, 0.75, na.rm = TRUE), 0)
```
\begin{center}
\textbf{Median number of measure, Diastolic or Systolic blood pressure, per patient (IQR) : `r median_val3` [`r q1_val3` - `r q3_val3`].} \\
\end{center}
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - NIMR}\\end{center}\n\n")
