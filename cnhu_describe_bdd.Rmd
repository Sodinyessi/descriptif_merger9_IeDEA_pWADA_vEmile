---
title: "\\textbf{CNHU COTONOU DESCRIPTION MERGER 9}"
author: "\\textbf{Emile Sodinyessi*}$^{1}$, Sophie Desmonde$^{1}$,  Karen Malateste$^{3}$,\ \n Valériane Leroy$^{1}$  \non behalf of the IeDEA West Africa Collaboration\n"
output:
  pdf_document:
    keep_md: true
...
$^{2}$Centre National Hospitalier Universitaire (CNHU), Cotonou, Bénin.\\
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("CNHU"))#, "CNHU COTONOU", "CHUY", "CIRBA", "CNHU", "GABRIEL", "KBTH", "NIMR", "TOKOIN", "YALGADO" ))   # filtre CNHU COTONOU
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - CNHU COTONOU - CNHU COTONOU - CHUY}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "CNHU COTONOU",     "CNHU COTONOU",     "CHUY")) %>%#,    "CIRBA",     "CNHU",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - CNHU COTONOU - CNHU COTONOU - CHUY**"
  )
...
tab03 <- base0 %>%
  ...
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - CNHU COTONOU**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including GABRIEL, MAKURDI, CHUY, JOS, AKURE, and CNHU COTONOU — are composed exclusively of Infected patients. In contrast, programs such as NIMR, CNHU COTONOU, CNHU, KBTH, TOKOIN, and YALGADO have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\vspace{1cm}
     ...
     -----------------------------               -----------------------------------             
           ...
           N == 780                                           N == 2,176
     -----------------------------               -----------------------------------               
       AF : 183 (23.4%)                            Born to mothers infected with HIV 
     ...
     ------------------------       -----------------------
       ART initiation               Unknown ART initiation  
         ...
         N == 632                        N == 148
     ------------------------       -----------------------
    Initiation of antiretroviral           AF : 1 (0.6%) *** 
     (therapy  >= 3 drugs)
       AF :   182 (28.8%) 
...
\newpage
\begin{landscape}
\section{Database closure dates CNHU COTONOU program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("CNHU COTONOU",  "CNHU COTONOU",    "CHUY",   "CIRBA",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "CNHU COTONOU") %>%
...
\subsubsection{Graphic CNHU COTONOU - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics CNHU COTONOU : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
gpl_cd4_2 <- wa_apin_full_m9_ped_apin %>%
  filter(enrol_y >= 2004 & enrol_y < 2020) %>%
  group_by(enrol_y) %>%
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - CNHU COTONOU}\\end{center}\n\n")
