---
title: "\\textbf{CHUC DESCRIPTION MERGER 9}"
...
$^2$Cocody University Hospital, Pediatric Department (CHUC), Abidjan, Côte d'Ivoire.\\
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("CHUC"))#, "CHUC", "CHUY", "CIRBA", "CNHU", "GABRIEL", "KBTH", "NIMR", "TOKOIN", "YALGADO" ))   # filtre CHUC
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - CHUC - CHUC - CHUY}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "CHUC",     "CHUC",     "CHUY")) %>%#,    "CIRBA",     "CNHU",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - CHUC - CHUC - CHUY**"
  )
...
tab03 <- base0 %>%
  ...
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - CHUC**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including GABRIEL, MAKURDI, CHUY, JOS, AKURE, and CHUC — are composed exclusively of Infected patients. In contrast, programs such as NIMR, CHUC, CNHU, KBTH, TOKOIN, and YALGADO have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\newpage
\begin{landscape}
\section{Database closure dates CHUC program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("CHUC",  "CHUC",    "CHUY",   "CIRBA",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "CHUC") %>%
...
\subsubsection{Graphic CHUC - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics CHUC : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - CHUC}\\end{center}\n\n")
