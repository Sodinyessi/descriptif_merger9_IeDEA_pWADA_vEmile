---
title: "\\textbf{GABRIEL DESCRIPTION MERGER 9}"
...
$^{2}$Gabriel Toure Children's Hospital (GABRIEL), Bamako, Mali.\\
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("GABRIEL"))#, "GABRIEL", "CHUY", "GABRIEL", "CNHU", "GABRIEL", "KBTH", "NIMR", "TOKOIN", "GABRIEL" ))   # filtre GABRIEL
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - GABRIEL - GABRIEL - CHUY}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "GABRIEL",     "GABRIEL",     "CHUY")) %>%#,    "GABRIEL",     "CNHU",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - GABRIEL - GABRIEL - CHUY**"
  )
...
#\subsubsection{Profile program : GABRIEL - CNHU - GABRIEL - JOS - KBTH}

tab02 <- base0 %>%
  filter(program %in% c("GABRIEL",     "CNHU",  "GABRIEL", "JOS",     "KBTH")) %>% #,     "NIMR",   "TOKOIN",      "UCH",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : JOS - KBTH - MAKURDI - NIMR - TOKOIN - UCH - GABRIEL**"
  )
...
#\subsubsection{Profile program : MAKURDI - NIMR - TOKOIN - UCH - GABRIEL}

tab002 <- base0 %>%
  filter(program %in% c("MAKURDI",  "NIMR",   "TOKOIN",      "UCH",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : MAKURDI - NIMR - TOKOIN - UCH - GABRIEL**"
  )
...
tab03 <- base0 %>%
  ...
  #filter(program %in% c("JOS",     "KBTH",  "MAKURDI",     "NIMR",   "TOKOIN",      "UCH",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "overall", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - GABRIEL**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including GABRIEL, MAKURDI, CHUY, JOS, AKURE, and GABRIEL — are composed exclusively of Infected patients. In contrast, programs such as NIMR, GABRIEL, CNHU, KBTH, TOKOIN, and GABRIEL have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\newpage
\begin{landscape}
\section{Database closure dates GABRIEL program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("GABRIEL",  "GABRIEL",    "CHUY",   "GABRIEL",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
tbl_char2 <- tbl_inf_charc %>%
  filter(program %in% c("GABRIEL", "KBTH",  "NIMR", "TOKOIN","GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
    include = c("sex_at_birth", "hiv_type", age_med_at_enroL_yrs, "year_of_inclusion_to_cohort", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "GABRIEL") %>%
...
\subsubsection{Graphic GABRIEL - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics GABRIEL : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
gpl_cd4_2 <- wa_apin_full_m9_ped_apin %>%
  filter(enrol_y >= 2009 & enrol_y < 2024) %>%
  group_by(enrol_y) %>%
...
wh_plot <- ggplot(df_long, aes(x = year, y = count, fill = indicator)) +
  ...
  scale_x_continuous(
    breaks = 2006:2023,       # affiche toutes les années
    limits = c(2006, 2023)    # borne l’axe X à 2006–2024
  ) +
  theme(
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - GABRIEL}\\end{center}\n\n")
