---
title: "\\textbf{KBTH DESCRIPTION MERGER 9}"
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("KBTH"))#, "KBTH", "KBTH", "KBTH", "CNHU", "KBTH", "KBTH", "KBTH", "TOKOIN", "KBTH" ))   # filtre KBTH
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - KBTH - KBTH - KBTH}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "KBTH",     "KBTH",     "KBTH")) %>%#,    "KBTH",     "CNHU",  "KBTH")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - KBTH - KBTH - KBTH**"
  )
...
#\subsubsection{Profile program : KBTH - CNHU - KBTH - JOS - KBTH}

tab02 <- base0 %>%
  filter(program %in% c("KBTH",     "CNHU",  "KBTH", "JOS",     "KBTH")) %>% #,     "KBTH",   "TOKOIN",      "UCH",  "KBTH")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : JOS - KBTH - MAKURDI - KBTH - TOKOIN - UCH - KBTH**"
  )
...
#\subsubsection{Profile program : MAKURDI - KBTH - TOKOIN - UCH - KBTH}

tab002 <- base0 %>%
  filter(program %in% c("MAKURDI",  "KBTH",   "TOKOIN",      "UCH",  "KBTH")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : MAKURDI - KBTH - TOKOIN - UCH - KBTH**"
  )
...
tab03 <- base0 %>%
  ...
  #filter(program %in% c("JOS",     "KBTH",  "MAKURDI",     "KBTH",   "TOKOIN",      "UCH",  "KBTH")) %>%
  ungroup() %>%
  tbl_summary(by = "overall", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - KBTH**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including KBTH, MAKURDI, KBTH, JOS, AKURE, and KBTH — are composed exclusively of Infected patients. In contrast, programs such as KBTH, KBTH, CNHU, KBTH, TOKOIN, and KBTH have a substantial proportion of HIV-Exposed patients, with KBTH and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\newpage
\begin{landscape}
\section{Database closure dates KBTH program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("KBTH",  "KBTH",    "KBTH",   "KBTH",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
tbl_char2 <- tbl_inf_charc %>%
  filter(program %in% c("KBTH", "KBTH",  "KBTH", "TOKOIN","KBTH")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
    include = c("sex_at_birth", "hiv_type", age_med_at_enroL_yrs, "year_of_inclusion_to_cohort", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "KBTH") %>%
...
\subsubsection{Graphic KBTH - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics KBTH : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
gpl_cd4_2 <- wa_apin_full_m9_ped_apin %>%
  filter(enrol_y >= 2006 & enrol_y < 2018) %>%
  group_by(enrol_y) %>%
...
graph_m6 <- bdd_graph_m6 %>%
  ...
  filter(y_art_m6 >= 2011 & y_art_m6 <= 2023) %>%
  group_by(y_art_m6) %>%
  summarise(
...
graph_m12_data <- bdd_graph_m12 %>%
  #filter(program == "ABEOKUTA") %>%
  mutate(f_art_year = year(f_art_d)) %>%
  filter(y_art_m12 >= 2011 & y_art_m12 <= 2023) %>%
  group_by(y_art_m12) %>%
  summarise(
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - KBTH}\\end{center}\n\n")
