---
title: "\\textbf{CHUY DESCRIPTION MERGER 9}"
...
$^{2}$Centre Hospitalier Universitaire de Yopougon (CHUY), Abidjan, Côte d'Ivoire.\\
...
for(nm in tbl_names){
  ...
  df_f <- dplyr::filter(df_f, program %in% c("CHUY"))#, "CHUY", "CHUY", "CHUY", "CNHU", "CHUY", "KBTH", "NIMR", "TOKOIN", "CHUY" ))   # filtre CHUY
...
```{r}
#\subsubsection{Profile program : ABEOKUTA - AKURE - CHUY - CHUY - CHUY}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "CHUY",     "CHUY",     "CHUY")) %>%#,    "CHUY",     "CNHU",  "CHUY")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - CHUY - CHUY - CHUY**"
  )
...
#\subsubsection{Profile program : CHUY - CNHU - CHUY - JOS - KBTH}

tab02 <- base0 %>%
  filter(program %in% c("CHUY",     "CNHU",  "CHUY", "JOS",     "KBTH")) %>% #,     "NIMR",   "TOKOIN",      "UCH",  "CHUY")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : JOS - KBTH - MAKURDI - NIMR - TOKOIN - UCH - CHUY**"
  )
...
#\subsubsection{Profile program : MAKURDI - NIMR - TOKOIN - UCH - CHUY}

tab002 <- base0 %>%
  filter(program %in% c("MAKURDI",  "NIMR",   "TOKOIN",      "UCH",  "CHUY")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : MAKURDI - NIMR - TOKOIN - UCH - CHUY**"
  )
...
tab03 <- base0 %>%
  ...
  #filter(program %in% c("JOS",     "KBTH",  "MAKURDI",     "NIMR",   "TOKOIN",      "UCH",  "CHUY")) %>%
  ungroup() %>%
  tbl_summary(by = "overall", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - CHUY**"
  )
...
#cat("The graph shows the distribution of patients by program and infection status. Several sites  including CHUY, MAKURDI, CHUY, JOS, AKURE, and CHUY — are composed exclusively of Infected patients. In contrast, programs such as NIMR, CHUY, CNHU, KBTH, TOKOIN, and CHUY have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\newpage
\begin{landscape}
\section{Database closure dates CHUY program}
```{r}
...
tbl_char1 <- tbl_inf_charc %>%
  filter(program %in% c("CHUY",  "CHUY",    "CHUY",   "CHUY",    "CNHU")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
...
tbl_char2 <- tbl_inf_charc %>%
  filter(program %in% c("CHUY", "KBTH",  "NIMR", "TOKOIN","CHUY")) %>%
  ungroup() %>%
  tbl_summary(
    by = "program", 
    include = c("sex_at_birth", "hiv_type", age_med_at_enroL_yrs, "year_of_inclusion_to_cohort", 
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "CHUY") %>%
...
\subsubsection{Graphic CHUY - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics CHUY : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - CHUY}\\end{center}\n\n")
