---
title: "\\textbf{IeDEA pWADA DESCRIPTION MERGER 9}"
...
$^4$Centre de Prise en Charge, de Recherche et de Formation (CEPREF), Abidjan, Côte d'Ivoire.\\
...
$^{10}$Bordeaux Population Health - Global Health in the Global South, Université de Bordeaux.\\
...
$^{16}$Centre Hospitalier Universitaire de Yopougon (CHUY), Abidjan, Côte d'Ivoire.\\
...
```{r eval=FALSE, fig.align='center', include=FALSE, out.width='75%'}
...
\subsubsection{Profile program : ABEOKUTA - AKURE - CEPREF - CHUC - CHUY}
...
```{r baseline-characteristics00, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
tab01 <- base0 %>%
  filter(program %in% c("ABEOKUTA",    "AKURE",   "CEPREF",     "CHUC",     "CHUY")) %>%#,    "CIRBA",     "CNHU",  "GABRIEL")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
  modify_spanning_header(
    #update = all_stat_cols() ~ "**Profile : ABEOKUTA - AKURE - CEPREF - CHUC - CHUY**"
  )
...
tab01_kable
#cat("\\begin{center}\\textbf{Cohort Profile : Number of patients by group and by program}\\end{center}\n\n")
```


\subsubsection{Profile program : CIRBA - CNHU - GABRIEL - JOS - KBTH}
```{r baseline-characteristics00K1, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
tab02 <- base0 %>%
  filter(program %in% c("CIRBA",     "CNHU",  "GABRIEL", "JOS",     "KBTH")) %>% #,     "NIMR",   "TOKOIN",      "UCH",  "YALGADO")) %>%
  ungroup() %>%
  tbl_summary(by = "program", include = c("patient_status")) %>%
  #add_overall(last = TRUE)|> 
  bold_labels() |> 
...
```{r baseline-characteristics00K2, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
tab002 <- base0 %>%
...
\newpage
\begin{landscape}
\subsubsection{Distribution of patients by status for overall cohorte}
...
tab03 <- base0 %>%
  ...
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Cohort Profile - all IeDEA pwada**"
  )
...
base1 <- base1 %>%
  mutate(
    ...
    groupe1_label = factor(groupe1_label, levels = c("Infected", "HIV-Exposed"))
  )

# 2. Créer la table des effectifs complets
counts2 <- base1 %>%
  count(program, groupe1_label, name = "n") %>%
  tidyr::complete(program, groupe1_label, fill = list(n = 0))

# 3. Tracer le graphique
wa_apin_plot_0 <- ggplot(counts2, aes(x = program, y = n, fill = groupe1_label)) +
  geom_col(position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = n, group = groupe1_label),
    position = position_dodge(width = 0.9),
    vjust = -0.3, #avant :: vjust = -0.3, hjust = 0.5, après vjust = 0.3, hjust = -00.5,  # attention : pour coord_flip
    hjust = 0.5,
    size = 3.5,
    fontface = "bold"
  ) +
  ...
  scale_fill_manual(values = c("#084594", "#CB181D")) +
...
cat("The graph shows the distribution of patients by program and infection status. Several sites  including GABRIEL, MAKURDI, CHUY, JOS, AKURE, and CHUC — are composed exclusively of Infected patients. In contrast, programs such as NIMR, CEPREF, CNHU, KBTH, TOKOIN, and YALGADO have a substantial proportion of HIV-Exposed patients, with NIMR and CNHU showing particularly high numbers in this group. These differences reflect the heterogeneous composition of patient populations across programs. \n")
...
\vspace{1cm}
     ...
     -----------------------------               -----------------------------------             
         ...
         N == 16,768                                          N == 14,145
     -----------------------------               -----------------------------------               
       AF : 4,542 (27%)                            Born to mothers infected with HIV 
     ...
     ------------------------       -----------------------
       ART initiation               Unknown ART initiation  
         ...
         N == 13,839                    N == 2,929
     ------------------------       -----------------------
    Initiation of antiretroviral           AF : 77 (3.5%) *** 
     (therapy  >= 3 drugs)
       AF :   4,465 (32%) 
...
\subsection{Frequency of children enrolled in ART Initiation and Unknown ART Initiation since 2006}
```{r, results='asis', echo=FALSE, out.width="100%"}
...
\newpage
\begin{landscape}
\section{Database closure dates of IeDEA centers with APIN program sites}
...
wa_apin_surv_full <- bdd_cum_inc %>% #filter(program == "CEPREF") %>%
...
\subsubsection{Graphic IeDEA - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics IeDEA : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
```{r, echo=FALSE, fig.width=12, fig.height=8, fig.landscape=TRUE}
...
\begin{landscape}
\newpage
\section{ART INITIATION : N = 13,839}
...
graph_m6 <- bdd_graph_m6 %>%
  ...
  filter(y_art_m6 >= 2006 & y_art_m6 <= 2025) %>%
  group_by(y_art_m6) %>%
  summarise(
...
graph_m12_data <- bdd_graph_m12 %>%
  #filter(program == "ABEOKUTA") %>%
  mutate(f_art_year = year(f_art_d)) %>%
  filter(y_art_m12 >= 2006 & y_art_m12 <= 2025) %>%
  group_by(y_art_m12) %>%
  summarise(
...
wh_plot <- ggplot(df_long, aes(x = year, y = count, fill = indicator)) +
  ...
  scale_x_continuous(
    breaks = 2006:2024,       # affiche toutes les années
    limits = c(2006, 2024)    # borne l’axe X à 2006–2024
  ) +
  theme(
...
```
\begin{center}
\textbf{Median number of measure, Diastolic or Systolic blood pressure, per patient (IQR) : `r median_val` [`r q1_val` - `r q3_val`].}
\end{center}
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - all IeDEA pwada}\\end{center}\n\n")
