---
title: "\\textbf{CePReF ENFANT DESCRIPTION MERGER 9}"
...
$^2$Centre de Prise en Charge, de Recherche et de Formation (CEPREF), Abidjan, Côte d'Ivoire.\\
...
tab03 <- base0 %>%
  ...
  bold_labels() |> 
  modify_spanning_header(
    update = all_stat_cols() ~ "**Site Profile - CePReF**"
  )
...
\vspace{1cm}
     ...
     -----------------------------               -----------------------------------             
         ...
         N == -,---                                          N == -,---
     -----------------------------               -----------------------------------               
       AF : 355 (--%)                            Born to mothers infected with HIV 
...
\newpage
\begin{landscape}
\section{Database closure dates CePReF program}
```{r}
...
\subsubsection{Graphic CePReF - Overall : 2014 - 2021}
```{r}
...
\newpage
\subsubsection{Graphics CePReF : 2014-2015, 2016-2017, 2018-2019, 2020-2021}
...
gpl_cd4_2 <- wa_apin_full_m9_ped_apin %>%
  filter(enrol_y >= 2004 & enrol_y < 2022) %>%
  group_by(enrol_y) %>%
...
graph_m6 <- bdd_graph_m6 %>%
  ...
  filter(y_art_m6 >= 2009 & y_art_m6 <= 2023) %>%
  group_by(y_art_m6) %>%
  summarise(
...
graph_m12_data <- bdd_graph_m12 %>%
  #filter(program == "ABEOKUTA") %>%
  mutate(f_art_year = year(f_art_d)) %>%
  filter(y_art_m12 >= 2009 & y_art_m12 <= 2023) %>%
  group_by(y_art_m12) %>%
  summarise(
...
tab_lab3_kable1
#cat("\\begin{center}\\textbf{Laboratory measurements - CePReF}\\end{center}\n\n")
